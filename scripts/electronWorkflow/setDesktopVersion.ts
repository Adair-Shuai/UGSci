...
function updatePackageJson() {
  ...
  try {
    ...
    switch (releaseType) {
      case 'stable': {
        packageJson.productName = 'UGSci';
        packageJson.name = 'ugsci-desktop';
        console.log('🌟 Setting as Stable version.');
        break;
      }
      case 'beta': {
        packageJson.productName = 'UGSci-Beta'; // Or 'LobeHub-Beta' if preferred
        packageJson.name = 'ugsci-desktop-beta'; // Or 'lobehub-desktop' if preferred
        console.log('🧪 Setting as Beta version.');
        updateAppIcon('beta');
        break;
      }
      case 'nightly': {
        packageJson.productName = 'UGSci-Nightly'; // Or 'LobeHub-Nightly'
        packageJson.name = 'ugsci-desktop-nightly'; // Or 'lobehub-desktop-nightly'
      ...
      case 'canary': {
        packageJson.productName = 'UGSci';
        packageJson.name = 'ugsci-desktop-canary';
